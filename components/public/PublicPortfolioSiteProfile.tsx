    ...
    >
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-black/[.06] bg-white/80 px-6 py-4 backdrop-blur sm:px-12 dark:border-white/[.08] dark:bg-zinc-950/80">
        ...
        <nav className="hidden items-center gap-8 text-sm sm:flex">
          ...
          {(site.profile?.description || site.profile?.logoUrl) && <a href="#about">{t.nav.about}</a>}
          {hasWork && <a href="#work">{t.nav.projects}</a>}
      ...
      {hasWork && (
        <section id="work" className="px-6 py-14 sm:px-12">
          <div className="mx-auto w-full max-w-6xl">
            <Reveal>
              <p className="text-xs font-semibold uppercase tracking-widest text-[var(--accent-solid)]">{t.section.featured}</p>
              <h2 className="mt-2 text-3xl font-bold">{t.nav.projects}</h2>
            </Reveal>
      ...
      <section id="contact" className="px-6 py-14 sm:px-12">
        <Reveal className="mx-auto w-full max-w-6xl">
          ...
          >
            <div>
              <h2 className="text-3xl font-bold">{t.contact.letsWorkTogether}</h2>
              <p className="mt-2 max-w-sm text-sm text-zinc-500 dark:text-zinc-400">
                {t.contact.haveProjectInMind}
              </p>
              {whatsappNumber && (
                <a
                  href={whatsappUrl(whatsappNumber, inquiryMessage)}
